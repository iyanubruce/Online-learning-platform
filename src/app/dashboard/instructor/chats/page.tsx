      ...
      <div className="col-span-7 rounded-lg bg-searchGray flex flex-col justify-end p-3">
        <div className="flex-grow max-h-[26rem] flex flex-col overflow-scroll pb-3">
          {chats.map((chat, index) => (
            <div
              key={index}
              className={`max-w-[25ch] text-pretty text-justify border border-neutral-500 p-2 rounded-md ${chat.from ? "self-start" : "self-end"}`}
            >
              <span>{chat.chat}</span>
            </div>
          ))}
        ...
        <form className="w-full h-40 rounded-md bg-[#161616] flex flex-col">
          <textarea
            name="chat"
            id=""
            className="bg-transparent w-full h-1/2"
          ></textarea>
