...
export default function page() {
  ...
  return (
    <div className="text-white p-10">
      <button
        className={`p-2 bg-btnColor rounded`}
        onClick={() =>
      ...
      <div className="w-full border border-gray-500 mt-7 h-[500px] rounded p-5 flex-col flex">
        <div className="flex flex-col pb">
          ...
          <span className="text-sm pb-2">
            {" "}
            Manage your Courses and veiw their sales and ratings
          </span>
        </div>
        <div className="w-full h-[85%] border border-gray-500 p-2">
          ...
          <div className="overflow-y-scroll h-[90%]">
            {orders.map((course, index) => (
              <Link
                href={`/dashboard/instructor/${username}/create-course/editcourse/${course.name}`}
                key={index}
                className={`${styles.list} ${styles.content}`}>
                <div>
                  <Image
                    src={course.imageurl}
                    width={60}
                ...
                <div className="grid place-content-center">{course.price}</div>
                <div className="grid place-content-center">
                  {course.totalSales}
                </div>
                <div className="grid place-content-center">
